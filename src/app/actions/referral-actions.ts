...
export async function applyReferralOnSignup(userId: string, referralCode: string) {
  try {
    ...
    await db.update(user).set({ referredByUserId: referrer.id }).where(eq(user.id, userId));

    // 2. Look up active reward config for ON_REGISTRATION
    const config = await db.query.rewardConfig.findFirst({
      where: and(eq(rewardConfig.triggerEvent, "ON_REGISTRATION"), eq(rewardConfig.isActive, true)),
    });
...
export async function issueBookingReward(refereeId: string) {
  try {
    ...
    const config = await db.query.rewardConfig.findFirst({
      where: and(eq(rewardConfig.triggerEvent, "ON_BOOKING"), eq(rewardConfig.isActive, true)),
    });
...
export async function issueSpaReward(refereeId: string) {
  try {
    ...
    const config = await db.query.rewardConfig.findFirst({
      where: and(eq(rewardConfig.triggerEvent, "ON_SPA_SIGNED"), eq(rewardConfig.isActive, true)),
    });
...
async function checkAndIssueMilestoneRewards(referrerId: string) {
  ...
  const issuedTierIds = new Set(existingMilestones.map((m) => m.notes).filter(Boolean));
