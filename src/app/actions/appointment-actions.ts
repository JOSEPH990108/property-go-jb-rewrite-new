...
export async function generateDemoAppointments() {
  ...
  try {
    ...
    if (projectList.length === 0)
      return { success: false, error: "System Error: No projects found" };
...
export async function scanAppointment(qrToken: string) {
  ...
  try {
    ...
    if (!completedStatus || !confirmedStatus)
      return { success: false, error: "System Error: Status not found" };
...
export async function processNoShows() {
  ...
  try {
    ...
    const expiredAppointments = await db.query.appointments.findMany({
      where: and(eq(appointments.statusId, confirmedStatus), lt(appointments.scheduledAt, cutoff)),
    });
