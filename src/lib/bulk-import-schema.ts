...
const optionalYesNoString = (defaultValue: boolean, falseValue: string) =>
  z
    .preprocess(
      ...
      z
        .string()
        .regex(/^(yes|no)$/i, "Value must be yes or no")
        .optional(),
    )
    .transform((value) => {
...
const optionalBooleanString = (defaultValue: boolean) =>
  z
    .preprocess(
      ...
      z
        .string()
        .regex(/^(true|false|yes|no)$/i, "Value must be true, false, yes, or no")
        .optional(),
    )
    .transform((value) => {
      if (!value) return defaultValue;
...
export const BulkProjectSchema = z.object({
  ...
  launchYear: emptyStringToUndefined(
    z.string().regex(/^\d{4}$/, "Launch year must be a 4-digit year"),
  ),
  ...
  tenureExpiryDate: emptyStringToUndefined(
    z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Tenure expiry date must be in YYYY-MM-DD format"),
  ),
});
...
export const BulkSalesPackageSchema = z.object({
  ...
  validFrom: emptyStringToUndefined(
    z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "validFrom must be YYYY-MM-DD"),
  ),
  validTo: emptyStringToUndefined(
    z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "validTo must be YYYY-MM-DD"),
  ),
  isActive: optionalBooleanString(true),
});
...
export const BulkTowerSpecialFloorSchema = z.object({
  ...
  projectSlug: z.string().min(1, "Project slug is required"),
  towerNumber: z.string().min(1, "Tower number is required"),
  floorKind: z.enum(["breaktank", "facility"], {
    message: "floorKind must be 'breaktank' or 'facility'",
  }),
