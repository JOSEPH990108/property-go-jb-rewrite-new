...
export function findTopMatches(state: MatchmakerState, allLayouts: LayoutData[]): PropertyMatch[] {
  ...
  const filtered = allLayouts.filter((layout) => {
    // Budget Buffer +10%
    const maxBudgetWithBuffer = state.maxBudget * 1.1;
...
function calculateScore(state: MatchmakerState, layout: LayoutData): LayoutScoreDetail {
  ...
  if (state.purchasePurpose === "Own Stay") {
    if (state.householdSize && state.householdSize <= 2)
      desiredBeds = 2; // 1-2 usually fit in 1-2 beds
    ...
    if (state.investmentGoal === "Yield" && layout.builtUpSqft < 800) {
      ...
      state.investmentGoal === "Appreciation" &&
      (layout.propertyType === "Landed" || layout.builtUpSqft > 1000)
    ) {
      featureScore += 10;
  ...
  if (
    state.propertyTypePreference !== "Open" &&
    layout.propertyType === state.propertyTypePreference
  ) {
    featureScore += 30;
  ...
  if (state.purchasePurpose === "Own Stay") {
    ...
    if (state.priority === "Resale Value") {
      ...
      breakdown.push(
        `Hybrid Bonus (Resale Focus): Yield (+${yieldBonus}) + Comfort (+${comfortBonus})`,
      );
    } else {
      // Comfort Focus
      ...
      purposeBonus += yieldBonus + comfortBonus;
      breakdown.push(
        `Hybrid Bonus (Comfort Focus): Comfort (+${comfortBonus}) + Yield (+${yieldBonus})`,
      );
    }
  }
