...
export const Carousel: React.FC<CarouselProps> = ({
  ...
  return (
    <SwipeWrapper
      ...
      className="perspective-container relative flex h-full w-full items-center justify-center overflow-hidden"
    >
      <style>{`
      ...
      <AnimatePresence>
        {selectedIndex !== null && (
          <motion.div
            className="absolute inset-0 z-40 cursor-pointer bg-black/60 backdrop-blur-sm"
      ...
      {/* Rotating Ring */}
      <motion.div
        className="preserve-3d relative flex items-center justify-center"
        style={{
          rotateX: tilt,
        ...
        {selectedIndex !== null && (
          <div className="pointer-events-none absolute inset-0 z-50 flex items-center justify-center">
            <motion.div
              ...
              className="pointer-events-auto relative cursor-default outline-none"
              animate={{
                rotateX: 0,
...
const CarouselItem: React.FC<CarouselItemProps> = ({
  ...
  return (
    <div
      className="preserve-3d absolute flex cursor-pointer items-center justify-center"
      style={{
    ...
    >
      <div
        className={`transition-opacity duration-300 ${isSelected ? "pointer-events-none opacity-0" : "opacity-100 hover:scale-105"}`}
      >
        <motion.div layoutId={`card-${index}`}>{child}</motion.div>
      </div>
    </div>
  );
};
