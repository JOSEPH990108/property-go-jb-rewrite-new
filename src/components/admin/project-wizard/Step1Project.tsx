  ...
  return (
    <div className="space-y-8">
      <SectionHeading
        ...
        subtitle="Core identity and classification for this project."
      />
      ...
      <SubSection title="Basic Info">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <FieldInput
            ...
            value={data.slug}
            onChange={set("slug")}
            placeholder="e.g. m-grand-minori"
          />
          <FieldInput
            label="Name"
            required
      ...
      <SubSection title="Classification">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <FieldSelect
            ...
            value={data.propertyCategoryId}
            onChange={set("propertyCategoryId")}
            options={options.propertyCategories}
          />
          <FieldSelect
            label="Property Type"
            value={data.propertyTypeId}
            onChange={set("propertyTypeId")}
            options={options.propertyTypes}
          />
          <FieldSelect
            label="Project Status"
            value={data.projectStatusId}
            onChange={set("projectStatusId")}
            options={options.projectStatuses}
          />
          <FieldSelect
            label="Tenure Type"
            required
            value={data.tenureTypeId}
            onChange={set("tenureTypeId")}
            options={options.tenureTypes}
          />
          <FieldSelect
            label="Title Type"
            value={data.titleTypeId}
            onChange={set("titleTypeId")}
            options={options.titleTypes}
          />
          <FieldInput
            label="Green Certification"
            value={data.greenCertification}
            onChange={set("greenCertification")}
            placeholder="e.g. GREENRE"
          />
        </div>
      </SubSection>
      ...
      <SubSection title="Fees & Details">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <FieldInput
            ...
            value={data.landAreaAcres}
            onChange={set("landAreaAcres")}
          ...
          <FieldInput
            ...
            placeholder="e.g. 1733"
            type="number"
          />
          <FieldInput
            label="Launch Year"
            value={data.launchYear}
            onChange={set("launchYear")}
            placeholder="e.g. 2026"
            type="number"
          />
          <FieldInput
            label="Booking Fee (RM)"
          ...
          />
          <FieldInput
            label="Booking Fee Bumi (RM)"
            value={data.bookingFeeBumi}
            onChange={set("bookingFeeBumi")}
            placeholder="e.g. 1000"
            type="number"
          />
          <FieldInput
            label="Maintenance Fee (per sqft)"
            value={data.maintenanceFeePerSqft}
            onChange={set("maintenanceFeePerSqft")}
            placeholder="e.g. 0.40"
            type="number"
          />
          <FieldInput
            label="Sinking Fund (per sqft)"
            value={data.sinkingFundPerSqft}
            onChange={set("sinkingFundPerSqft")}
      ...
      <SubSection title="Flags">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <FieldSwitch
            label="Foreigner Eligible"
            checked={data.isForeignerEligible}
            onChange={(v) => set("isForeignerEligible")(v)}
          />
          <FieldSwitch
            label="Gated Community"
            checked={data.isGatedCommunity}
            onChange={(v) => set("isGatedCommunity")(v)}
          />
          <FieldSwitch
            label="Hot Deal"
